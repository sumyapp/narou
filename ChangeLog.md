...
-----------------
#### 変更内容
- Ruby 3.4 以降の実行環境で `narou web` が起動しない問題を解消
- Bundler 経由の依存読み込みと Erubi への切り替えで最新の gem に追従
- `csv` `nkf` `ostruct` 等の分離された標準ライブラリを依存関係に追加
- rackup 不在環境でも Sinatra 4.x を起動可能にするモンキーパッチを追加
